...
public class GVITeleporter extends Teleporter {
	...
	public GVITeleporter(WorldServer worldIn) {
		super(worldIn);
	...
	public void placeInPortal(Entity entityIn, float rotationYaw) {
		if (this.worldServerInstance.provider.getDimension() != 1) {
			if (!this.placeInExistingPortal(entityIn, rotationYaw)) {
				while (!this.makePortal(entityIn)) {}
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? GVIBlockManager.FOLIGENU_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
			
			entityIn.playSound(SoundEvents.BLOCK_PORTAL_TRAVEL, 1.0F, 0.5F + new Random().nextFloat());
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1) {
						...
						if (this.worldServerInstance.getBlockState(blockpos).getBlock() == GVIBlockManager.GVI_PORTAL) {
							while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == GVIBlockManager.GVI_PORTAL) {
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D) {
			...
			if (this.worldServerInstance.getBlockState(((BlockPos) object).west()).getBlock() == GVIBlockManager.GVI_PORTAL) {
				enumfacing = EnumFacing.NORTH;
			}
			
			if (this.worldServerInstance.getBlockState(((BlockPos) object).east()).getBlock() == GVIBlockManager.GVI_PORTAL) {
				enumfacing = EnumFacing.SOUTH;
			}
			
			if (this.worldServerInstance.getBlockState(((BlockPos) object).north()).getBlock() == GVIBlockManager.GVI_PORTAL) {
				enumfacing = EnumFacing.EAST;
			}
			
			if (this.worldServerInstance.getBlockState(((BlockPos) object).south()).getBlock() == GVIBlockManager.GVI_PORTAL) {
	...
	public boolean makePortal(Entity p_85188_1_) {
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						boolean flag = k3 < 0;
						this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? GVIBlockManager.FOLIGENU_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
		}
		
		IBlockState iblockstate = GVIBlockManager.GVI_PORTAL.getDefaultState().withProperty(BlockPortal.AXIS, l5 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
		...
		for (j3 = 0; j3 < 4; ++j3) {
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? GVIBlockManager.FOLIGENU_BLOCK.getDefaultState() : iblockstate, 2);
				}
			}
		...
		p_85188_1_.playSound(SoundEvents.BLOCK_GRASS_PLACE, 1.0F, 0.5F + new Random().nextFloat());
		
		return true;
	}
	
	/**
	 * called periodically to remove out-of-date GVIBlockManager.gvi_portal
	 * locations from the cache list. Argument par1 is a
