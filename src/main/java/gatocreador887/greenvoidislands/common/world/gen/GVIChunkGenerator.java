...
public class GVIChunkGenerator implements IChunkGenerator {
	/** RNG. */
	private Random rand;
	...
	double[] lperlin1NoiseArray;
	double[] lperlin2NoiseArray;
	...
	private MapGenBase caveGenerator = new MapGenGVICaves();
	// temporary variables used during event handling
	private int chunkX = 0;
	private int chunkZ = 0;
	
	public GVIChunkGenerator(World worldObjIn, long seed) {
		this.world = worldObjIn;
		this.rand = new Random(seed);
		this.lperlin1 = new NoiseGeneratorOctaves(this.rand, 16);
		this.lperlin2 = new NoiseGeneratorOctaves(this.rand, 14);
		this.perlin = new NoiseGeneratorOctaves(this.rand, 12);
	...
	public void setBlocksInChunk(int x, int z, ChunkPrimer primer) {
		...
		int k = 41;
		int l = i + 1;
		this.noiseArray = this.genNoiseArray(this.noiseArray, x * i, 0, z * i, j, k, l);
		
		for (int i1 = 0; i1 < i; ++i1) {
			for (int j1 = 0; j1 < i; ++j1) {
				for (int k1 = 0; k1 < 40; ++k1) {
					double d0 = 0.25D;
					double d1 = this.noiseArray[((i1 + 0) * l + j1 + 0) * k + k1 + 0];
					double d2 = this.noiseArray[((i1 + 0) * l + j1 + 1) * k + k1 + 0];
					double d3 = this.noiseArray[((i1 + 1) * l + j1 + 0) * k + k1 + 0];
					double d4 = this.noiseArray[((i1 + 1) * l + j1 + 1) * k + k1 + 0];
					double d5 = (this.noiseArray[((i1 + 0) * l + j1 + 0) * k + k1 + 1] - d1) * d0;
					double d6 = (this.noiseArray[((i1 + 0) * l + j1 + 1) * k + k1 + 1] - d2) * d0;
					double d7 = (this.noiseArray[((i1 + 1) * l + j1 + 0) * k + k1 + 1] - d3) * d0;
					double d8 = (this.noiseArray[((i1 + 1) * l + j1 + 1) * k + k1 + 1] - d4) * d0;
	...
	private double[] genNoiseArray(double[] noiseArray, int x, int y, int z, int scaleX, int scaleY, int scaleZ) {
		...
		double d1 = 684.412D;
		d0 = d0 * 2.0D;
		this.perlinNoiseArray = this.perlin.generateNoiseOctaves(this.perlinNoiseArray, x, y, z, scaleX, scaleY, scaleZ, d0 / 80.0D, d1 / 160.0D, d0 / 80.0D);
		this.lperlin1NoiseArray = this.lperlin1.generateNoiseOctaves(this.lperlin1NoiseArray, x, y, z, scaleX, scaleY, scaleZ, d0, d1, d0);
		this.lperlin2NoiseArray = this.lperlin2.generateNoiseOctaves(this.lperlin2NoiseArray, x, y, z, scaleX, scaleY, scaleZ, d0, d1, d0);
		int i = x / 2;
		int j = z / 2;
		int k = 0;
		...
		for (int l = 0; l < scaleX; ++l) {
			for (int i1 = 0; i1 < scaleZ; ++i1) {
				...
				for (int j1 = 0; j1 < scaleY; ++j1) {
					double d2 = 0.0D;
					double d3 = this.lperlin1NoiseArray[k] / 512.0D;
					double d4 = this.lperlin2NoiseArray[k] / 512.0D;
					double d5 = (this.perlinNoiseArray[k] / 10.0D + 1.0D) / 2.0D;
