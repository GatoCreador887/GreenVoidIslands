...
public class BlockShronkLeaves extends BlockLeaves {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if ((worldIn.getBlockState(pos.west()).getMaterial() == Material.AIR || worldIn.getBlockState(pos.east()).getMaterial() == Material.AIR || worldIn.getBlockState(pos.north()).getMaterial() == Material.AIR || worldIn.getBlockState(pos.south()).getMaterial() == Material.AIR || worldIn.getBlockState(pos.up()).getMaterial() == Material.AIR || worldIn.getBlockState(pos.down()).getMaterial() == Material.AIR) && state.getValue(FRUIT_AGE) < 3 && rand.nextInt(10) == 0) {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (state.getValue(FRUIT_AGE) > 0) {
			...
			state.getBlock();
			Block.spawnAsEntity(world, pos.offset(side), new ItemStack(GVIItemManager.SHRONK, 1, state.getValue(FRUIT_AGE) - 1));
			return true;
		}
	...
	public void getDrops(net.minecraft.util.NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		ItemStack drop = new ItemStack(this.getItemDropped(state, rand, fortune), 1, 0);
		// if (!drop.isEmpty())
		// drops.add(drop);
