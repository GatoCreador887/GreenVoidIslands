...
public class BlockTallIslandGrass extends BlockBush implements IGrowable, net.minecraftforge.common.IShearable {
		...
		if (!worldIn.isRemote) {
			if (worldIn.getLightFromNeighbors(pos.up()) < 2 && worldIn.getBlockState(pos.up()).getLightOpacity(worldIn, pos.up()) > 2) {
				...
				if (worldIn.getLightFromNeighbors(pos.up()) >= 4) {
					...
					/*
					 * for (int i = 0; i < 4; ++i) { BlockPos blockpos = pos.add(rand.nextInt(3) -
					 * 1, rand.nextInt(5) - 3, rand.nextInt(3) - 1); if (blockpos.getY() >= 0 &&
					 * blockpos.getY() < 256 && !worldIn.isBlockLoaded(blockpos)) { return; }
					 * IBlockState iblockstate = worldIn.getBlockState(blockpos.up()); IBlockState
					 * iblockstate1 = worldIn.getBlockState(blockpos); if (iblockstate1.getBlock()
					 * == Blocks.DIRT && iblockstate1.getValue(BlockDirt.VARIANT) ==
					 * BlockDirt.DirtType.DIRT && worldIn.getLightFromNeighbors(blockpos.up()) >= 4
					 * && worldIn.getLightFromNeighbors(blockpos.up()) < 13 &&
					 * iblockstate.getLightOpacity(worldIn, pos.up()) <= 2) {
					 * worldIn.setBlockState(blockpos,
					 * GVIBlockManager.island_grass.getDefaultState()); } }
					 */
				}
			}
		}
	...
	public static enum EnumType implements IStringSerializable {
		GRASS(0, "tall_grass"), FERN(1, "fern"), DEAD_GRASS(2, "dead_tall_grass"), DEAD_FERN(3, "dead_fern");
