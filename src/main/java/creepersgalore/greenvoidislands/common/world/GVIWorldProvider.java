...
import creepersgalore.greenvoidislands.GreenVoidIslands;
...
import creepersgalore.greenvoidislands.common.world.biome.GVIBiomeProvider;
import creepersgalore.greenvoidislands.common.world.gen.GVIChunkProvider;
...
public class GVIWorldProvider extends WorldProvider {
	...
	public static DimensionType dtype = DimensionType.register("Green Void Islands", "green_void_islands", 7,
			GVIWorldProvider.class, false);
	
          protected float[] originalLightBrightnessTable = new float[16];
          
		@Override
		public void init() {
			this.biomeProvider = new GVIBiomeProvider(this.world.getSeed(), world.getWorldInfo().getTerrainType());
	    ...
	    {
	        return getSkyColorBody(cameraEntity, partialTicks);
	    }
     ...
     protected void generateLightBrightnessTable() {
          /*float minBrightness = (float) (1.0F / 10000000.0F * Math.pow(105, 3.25F) + 0.002F);
          for(int i = 0; i <= 15; i++) {
               float f1 = 1F - (i*i) / (15F*15F);
               this.lightBrightnessTable[i] = ((1F - f1) / (f1 * 6F + 1F) * (1F - minBrightness) + minBrightness);
          }
          System.arraycopy(this.lightBrightnessTable, 0, this.originalLightBrightnessTable, 0, 16);*/super.generateLightBrightnessTable();
     }
