            ...
            for (int j = MathHelper.floor(-f); j <= MathHelper.ceil(f); ++j)
            {
                for (int k = MathHelper.floor(-f); k <= MathHelper.ceil(f); ++k)
                {
                    if ((float)(j * j + k * k) <= (f + 1.0F) * (f + 1.0F))
                    {
                        	...
                        	this.setBlockAndNotifyAdequately(worldIn, position.add(j, i, k), worldIn.getBiome(position).topBlock);
                        	this.setBlockAndNotifyAdequately(worldIn, position.add(j, i - 1, k), Blocks.DIRT.getDefaultState());
