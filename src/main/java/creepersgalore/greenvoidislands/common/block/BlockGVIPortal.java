...
import creepersgalore.greenvoidislands.common.world.GVITeleporter;
...
public class BlockGVIPortal extends Block {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != GVIBlockManager.foligenu_block) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == GVIBlockManager.foligenu_block) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != GVIBlockManager.foligenu_block || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != GVIBlockManager.foligenu_block || getBlock(par1World, par2 + b0, par3, par4
										+ b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
         ...
         public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state)
    {
		...
		public void onEntityCollidedWithBlock(World par1World, BlockPos pos, IBlockState state, Entity par5Entity) {
			...
			} else if (thePlayer.dimension != 7) {
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 7,
						new GVITeleporter(thePlayer.mcServer.getWorld(7)));
			} 
                    ...
                    thePlayer.timeUntilPortal = 10;
                     thePlayer.mcServer.getPlayerList().transferPlayerToDimension(thePlayer, 0,
                               new GVITeleporter(thePlayer.mcServer.getWorld(0)));
                theEntity.setPortal(pos);
               } 
			}
		}
